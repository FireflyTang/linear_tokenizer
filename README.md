...
### 各模型独立拟合 — tol=5% vs tol=10% 对比

每个模型用专属系数，在保证率与过高估之间取不同平衡：

| 模型 | tol | 保证率 | 均高估 | P90 | 最大高估 | 低估样本数 | 最大低估 |
|------|-----|--------|--------|-----|---------|-----------|---------|
| GLM-5 | 5% | 96.2% | +15.8% | +29.8% | +73.4% | 42 | −5.0% |
| DSV-V3.2 | 5% | 96.2% | +16.2% | +30.8% | +82.7% | 42 | −5.0% |
| Kimi-K2.5 | 5% | 97.7% | +15.3% | +25.6% | +63.9% | 26 | −5.0% |
| MiniMax-M2.5 | 5% | 97.9% | +17.7% | +30.4% | +92.2% | 24 | −5.0% |
| GLM-5 | 10% | 84.7% | +9.7% | +23.0% | +64.2% | 171 | −10.0% |
| DSV-V3.2 | 10% | 86.8% | +10.1% | +23.9% | +73.1% | 148 | −10.0% |
| Kimi-K2.5 | 10% | 84.4% | +9.2% | +19.0% | +55.3% | 174 | −10.0% |
| MiniMax-M2.5 | 10% | 92.7% | +11.5% | +23.5% | +82.1% | 82 | −10.0% |

tol=5% 保证率高 10–13 个百分点（96–98% vs 84–93%），代价是均高估多约 +6pp（~+16% vs ~+10%）。

各模型专属系数：

```python
# 各模型独立系数（tol=5%，运行 _refit_filtered.py 重新拟合）
GLM_UPPER_RELAXED_5  = Coeffs(cjk=0.7225, letter=0.1530, digit=1.4497, punct=1.0010, space=0.1252, word=0.1574)
DSV_UPPER_RELAXED_5  = Coeffs(cjk=0.5885, letter=0.1583, digit=1.5859, punct=1.1659, space=0.1121, word=0.1182)
KIMI_UPPER_RELAXED_5 = Coeffs(cjk=0.7726, letter=0.1476, digit=1.4159, punct=0.7995, space=0.1219, word=0.3979)
MMAX_UPPER_RELAXED_5 = Coeffs(cjk=0.6434, letter=0.2028, digit=1.7761, punct=1.0950, space=0.0931, word=0.2923)

# 各模型独立系数（tol=10%，运行 _refit_filtered.py 重新拟合）
...
| 已知目标模型，97% 保证率 | 各模型独立系数 tol=5% | 均高估 ~16%，保证率 96–98% |
| 已知目标模型，最小高估 | 各模型独立系数 tol=10% | 均高估 ~10%，保证率 84–93% |

关于共享 vs 独立系数：共享系数以 MiniMax（token 数最多的模型）为基准，对 GLM/DSV/Kimi 偏保守（多高估 ~20 个百分点）。如果只服务单一模型，建议使用各模型独立拟合的系数。tol=5% 是保证率与高估幅度的折中（96–98% 保证率，均高估 +15–18%）；tol=10% 将高估压到最低（均高估 +10%），但保证率降至 84–93%。
