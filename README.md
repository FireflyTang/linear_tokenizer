...
```python
from tokenizer_approx import estimate, UPPER_COEFFS, extract_features, estimate7, UPPER_COEFFS7
...
# 上界系数（100% 保证不低估，6 特征，最大高估 +112%）
estimate(text, coeffs=UPPER_COEFFS)   # → int（≥ 真实 tokenizer 的结果）

# 上界系数（100% 保证不低估，7 特征，最大高估 +102%）
estimate7(text, coeffs=UPPER_COEFFS7) # → int（digit 拆分，纯英文最大高估降至 +62%）
...
| 6 | LP 21 类（当前 UPPER_COEFFS，真实数据） | 100% | 36.9% | +112.3% |
| **7** | **7 特征 LP（digit_iso/digit_run 拆分）** | **100%** | **36.8%** | **+102.2%** |
